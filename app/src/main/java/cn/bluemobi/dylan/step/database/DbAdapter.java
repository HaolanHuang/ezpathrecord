package cn.bluemobi.dylan.step.database;
...
public class DbAdapter {
	...
	public static final String LIMIT = "0,1";
	...
	public List<PathRecord> queryRecordAll() {
		...
		while (allRecordCursor.moveToNext()) {
			...
			record.setDistance(allRecordCursor.getDouble(allRecordCursor
					.getColumnIndex(DbAdapter.KEY_DISTANCE)));
			record.setDuration(allRecordCursor.getDouble(allRecordCursor
	...
	public PathRecord queryRecordById(int mRecordItemId) {
		...
		if (cursor.moveToNext()) {
			record.setId(cursor.getInt(cursor
					.getColumnIndex(DbAdapter.KEY_ROWID)));
			record.setDistance(cursor.getDouble(cursor
					.getColumnIndex(DbAdapter.KEY_DISTANCE)));
			record.setDuration(cursor.getDouble(cursor
					.getColumnIndex(DbAdapter.KEY_DURATION)));
	...
	* */
	public PathRecord queryLastRecord(){
		Cursor cursor = db.query(RECORD_TABLE, getColumns(), null,
				null, null, null, ORDER_BY,LIMIT);
		PathRecord record = new PathRecord();
		if (cursor.moveToNext()) {
			record.setId(cursor.getInt(cursor
					.getColumnIndex(DbAdapter.KEY_ROWID)));
